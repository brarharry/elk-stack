...
Load balancing ensures that the application will be highly _____(redundant), in addition to restricting _____(accessibilty) to the network.
- _TODO: What aspect of security do load balancers protect? What is the advantage of a jump box?_

Integrating an ELK server allows users to easily monitor the vulnerable VMs for changes to the _____(system logs) and system _____(resources).
...
| ELK-Stack | ElasticSearch Stack | 10.2.0.4   | Linux            |

### Access Policies

The machines on the internal network are not exposed to the public Internet. 

Only the _____(jumpbox) machine can accept connections from the Internet. Access to this machine is only allowed from the following IP addresses:
- _TODO: Add whitelisted IP addresses_ (My personal machine: 130.212.94.94)

Machines within the network can only be accessed by _____(jumpbox: 20.150.136.43).
...
| ELK-VM   | No                  |     10.1.0.4(Private - JumpBox's IP)    	e  |
